...
{history_text}"""),
    ...
    for chunk in relevant_chunks:
        source = os.path.basename(chunk.metadata.get("source", "未知"))
        # 只保留真正相关的片段来源（内容和问题有关的）
        if source not in sources:
            sources.append(source)

    return answer, relevant_chunks, sources
...
def run_rag_chatbot():
    ...
    while True:
        ...
        print("\n检索中...")
        answer, chunks, sources = rag_answer(question, vectorstore, chat_history)

        print(f"\n回答：\n{answer}")

        # 来源从元数据直接提取，格式统一，不依赖模型
        print(f"\n📄 参考来源：{' | '.join(sources)}")
